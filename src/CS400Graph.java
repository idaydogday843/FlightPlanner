...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
	...
	protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
		if (!vertices.containsKey(start)) {
			throw new NoSuchElementException("No vertex containing \"" + start.toString() + "\" is found");
		}
		if (!vertices.containsKey(end)) {
			throw new NoSuchElementException("No vertex containing \"" + end.toString() + "\" is found");
		}

		Vertex startVertex = vertices.get(start);
		LinkedList<NodeType> passedVertices = new LinkedList<NodeType>();
		PriorityQueue<Path> allPaths = new PriorityQueue<Path>();
		PriorityQueue<Path> finishedPaths = new PriorityQueue<Path>();

		Vertex endVertex = vertices.get(end);
		Path completePath = new Path(startVertex);
		Path possiblePath = new Path(startVertex);
		...
		while (!startVertex.equals(endVertex)) {
			...
			for (int i = 0; i < currentVertex.edgesLeaving.size(); i++) {
				nextVertex = currentVertex.edgesLeaving.get(i).target;
				possiblePath = new Path(completePath, currentVertex.edgesLeaving.get(i));
				try {
					if (!passedVertices.contains(nextVertex.data)) {
			...
			try {
				if (allPaths.isEmpty() && finishedPaths.isEmpty()) {
					throw new NoSuchElementException(
							"No path between \"" + start.toString() + "\" to \"" + end.toString() + "\" is found");
				}
				possiblePath = allPaths.poll();
				completePath = possiblePath;
				if (completePath.end.equals(endVertex)) {
					finishedPaths.add(completePath);
					possiblePath = allPaths.poll();
					currentVertex = possiblePath.end;
				}
				if (!completePath.end.equals(endVertex)) {
					...
					currentVertex = completePath.end;
				}
